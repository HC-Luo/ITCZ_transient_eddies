#EOF analysis on TD waves at eastern Indian Ocean
import numpy as np 
import xarray as xr
import myfun as wf
...
def main():
    ...
    months = [1,2,11,12]
    fout0 =      f"EOF_sigtest_{BASIN}_NDJF.pdf"
    fout1 =     f"EOF_div_vMSE_{BASIN}_NDJF.pdf"
    fout2 = f"EOF_Ano_vMSE_{BASIN}_NDJF.pdf"
    ...
    if BASIN == "EP":
        ...
        BOUNDARY1 = [-15,30,60,100]
        BOUNDARY2 = [-10,20,85,90]
    elif BASIN == "NA":
    ...
    for i in years:
        ...
        for j in np.arange(1,13):
            ...
            for k in days:
                ...
                +year_month+k+"00_"+year_month+k+"23.nc"
                fname9 = "e5.oper.an.pl.128_129_z.ll025sc." \
                +year_month+k+"00_"+year_month+k+"23.nc"
                ...
                if np.isin(fname5,folder):
                    ...
                    fnames9.append(fpath4+year_month+"/"+fname9)
    ...
    del MSE850, uMSE_adjust, vMSE_adjust, U850, V850
    gc.collect()

    ds9   = read_data(fnames9,BOUNDARY,temporal_resolution,spatial_resolution) 
    ds9["longitude"] = (ds9["longitude"] + 180) % 360 - 180
    ds9 = ds9.sortby('longitude')
    ds9 = ds9.sel(level = 850,longitude=slice(BOUNDARY1[2], BOUNDARY1[3]))
    Z850 = xr.decode_cf(ds9,decode_times = True)['Z'] 
    ...
    Data7_filtered = fft_filter(
        Z850.chunk(dict(time=-1)),
        time_low=freq_low,  # 1/30 cycles/h ≈ 30-h period
        time_high=freq_hi,   # 1/5 cycles/h ≈ 5-h period
        lon_low=wavenumber_low,    # 1/20 cycles/degree ≈ 20-degree wavelength
        lon_high=wavenumber_hi     # 1/5 cycles/degree ≈ 5-degree wavelength
    ...
    Data7_filtered = Data7_filtered.sel(time = Data7_filtered.time.dt.month.isin(months))
    ...
    Data7_filtered = Data7_filtered - Data7_filtered.mean("time")
    ...
    scores, components, expvar = EOF(Data1_filtered.sel(latitude = slice(BOUNDARY2[0],BOUNDARY2[1]),longitude = slice(BOUNDARY2[2],BOUNDARY2[3])),
                                     n_modes = 5,test = True, fout = fout0)
    regression1 = xr.DataArray(data=np.zeros((3, len(Data7_filtered.latitude), len(Data7_filtered.longitude))),
                               coords={"mode": np.arange(1,4),
                                       "latitude": Data7_filtered.latitude,
                                       "longitude": Data7_filtered.longitude})
...
# EOF 
def EOF(var,n_modes = 3,test=False, **kwargs):
    model = xe.single.EOF(n_modes=n_modes)
        ...
        for i, (eig, err) in enumerate(zip(eigenvalues, errors), start=1):
            ...
            if i > 1:
                ...
                if (eig + err) >= (prev_eig - prev_err):
                    ...
                    if i == 2:
                        print("Recomputing using Varimax rotation...")
                        return rEOF(var,model,test, n_modes)
                else:
...
def rEOF(var,model,test=False,n_modes = 3, **kwargs):
    rotator = xe.single.EOFRotator(n_modes=n_modes)
    rotator.fit(model)
    expvar = rotator.explained_variance_ratio()
    components = rotator.components()
    scores = rotator.scores()


    if test:
        fout = kwargs.get('fout', 'EOF_sigtest.pdf')
        # Extract eigenvalues (λ) and get number of samples (N)
        eigenvalues = rotator.explained_variance()
        n_samples = len(var.time)
        errors = eigenvalues * np.sqrt(2 / n_samples)
        ...
        for i, (eig, err) in enumerate(zip(eigenvalues, errors), start=1):
            ...
            if i > 1:
                ...
                # Check overlap with previous mode
                if (eig + err) >= (prev_eig - prev_err):
                    print(f"  -> Mode {i} is still NOT distinct from Mode {i-1}")
                else:
                    print(f"  -> Mode {i} is now distinct from Mode {i-1}")
