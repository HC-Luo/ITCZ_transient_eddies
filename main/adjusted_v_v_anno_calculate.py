...
if __name__ == "__main__":
    ...
    years = [begin]
    fout1 = fpath_out+"v_adj/v_adj_"+f"{years[0]}.nc"
    fout2 = fpath_out+"v_adj/v_adj_anno_"+f"{years[0]}.nc"
    fout3 = fpath_out+"MSE/MSE_anno_"+f"{years[0]}.nc"
...
#
    ...
    mask = maskout(q,sp)#xr.ones_like(vMSE)#
    ...
    v_adj = v_adj.sel(time = TIME_mark)
    v_adj_anno = v_adj_anno.sel(time = TIME_mark)
    MSE_anno = MSE_anno.sel(time = TIME_mark)

    ds_out1 = xr.Dataset(data_vars = {"v_adj":(("time","level","latitude","longitude"),v_adj.data)},
                        coords = {"time":v_adj.time,
                                  "level":v_adj.level,
                                  "latitude":v_adj.latitude,
                                  "longitude":v_adj.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly adjusted v",
    ...
    ds_out2 = xr.Dataset(data_vars = {"v_adj_anno":(("time","level","latitude","longitude"),v_adj_anno.data)},
                        coords = {"time":v_adj_anno.time,
                                  "level":v_adj_anno.level,
                                  "latitude":v_adj_anno.latitude,
                                  "longitude":v_adj_anno.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly adjusted v anomalies",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"m s^-1"})
    ds_out2.to_netcdf(path = fout2) 
    del v_adj_anno,ds_out2
    gc.collect()

    ds_out3 = xr.Dataset(data_vars = {"MSE_anno":(("time","level","latitude","longitude"),MSE_anno.data)},
                        coords = {"time":MSE_anno.time,
                                  "level":MSE_anno.level,
                                  "latitude":MSE_anno.latitude,
                                  "longitude":MSE_anno.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly adjusted MSE anomalies",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"J kg^-1"})
    ds_out3.to_netcdf(path = fout3) 
    del MSE_anno,ds_out3
    gc.collect()
