#calculate MMC (column mean removed), stationary eddy and transient eddy component of meridional flux
import numpy as np 
import xarray as xr
import myfun as wf
...
if __name__ == "__main__":
    ...
    fout3 = fpath_out+"vMSE_col_stationary/vMSE_col_stationary_"+f"{years[0]}.nc"
    ...
    #vMSE_full = v_adj*MSE
    v_time_mean = running_mean(v_adj,temporal_resolution)
    v_mean = v_time_mean.mean("longitude",skipna=True)
    v_mean = xr.where(mask==1, v_mean,np.nan)
    v_mean = v_mean-wf.nantrapz(v_mean,v_mean.level,dim = "level")/sp
    MSE_time_mean = running_mean(MSE,temporal_resolution)
    ...
    vMSE_MMC = (v_mean*MSE_mean).broadcast_like(MSE)
    vMSE_transient = v_ano*MSE_ano
    del v_ano,MSE_ano
    vMSE_stationary = ((v_time_mean-v_mean)*(MSE_time_mean-MSE_mean)).mean("longitude",skipna=True).broadcast_like(MSE)
    del v_time_mean,MSE_time_mean,v_mean,MSE_mean,MSE
    gc.collect()
    ...
    vMSE_MMC_col = column_integrate(vMSE_MMC,mask).sel(time = TIME_mark)
    vMSE_transient_col = column_integrate(vMSE_transient,mask).sel(time = TIME_mark)
    vMSE_stationary_col = column_integrate(vMSE_stationary,mask).sel(time = TIME_mark)
    del vMSE_transient,vMSE_stationary,vMSE_MMC
    gc.collect()
    '''
    ds_out1 = xr.Dataset(data_vars = {"vMSE_full_col":(("time","latitude","longitude"),vMSE_full_col.data)},
                        coords = {"time":vMSE_full_col.time,
                                  "latitude":vMSE_full_col.latitude,
                                  "longitude":vMSE_full_col.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly column-integrated meridional moist static energy flux (full)",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"W m^-1"})
    ds_out1.to_netcdf(path = fout1) 
    del vMSE_full_col,ds_out1
    gc.collect()
    '''
    ds_out2 = xr.Dataset(data_vars = {"vMSE_MMC_col":(("time","latitude","longitude"),vMSE_MMC_col.data)},
                        coords = {"time":vMSE_MMC_col.time,
                                  "latitude":vMSE_MMC_col.latitude,
                                  "longitude":vMSE_MMC_col.longitude,},
                        attrs = {"description":"column-integrated meridional moist static energy flux (MMC)",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"W m^-1"})
    ds_out2.to_netcdf(path = fout2) 
    del vMSE_MMC_col,ds_out2
    ...
    ds_out3 = xr.Dataset(data_vars = {"vMSE_stationary_col":(("time","latitude","longitude"),vMSE_stationary_col.data)},
                        ...
                        attrs = {"description":"column-integrated meridional moist static energy flux (stationary)",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"W m^-1"})
    ...
    ds_out4 = xr.Dataset(data_vars = {"vMSE_transient_col":(("time","latitude","longitude"),vMSE_transient_col.data)},
                        coords = {"time":vMSE_transient_col.time,
                                  "latitude":vMSE_transient_col.latitude,
                                  "longitude":vMSE_transient_col.longitude,},
                        attrs = {"description":"column-integrated meridional moist static energy flux (transient)",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"W m^-1"})
    ds_out4.to_netcdf(path = fout4) 
    del vMSE_transient_col,ds_out4
    gc.collect()
