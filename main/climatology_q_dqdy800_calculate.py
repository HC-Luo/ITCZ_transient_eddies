#Composites of q and dqdy at 800
import numpy as np 
import xarray as xr
import myfun as wf
...
if __name__ == "__main__":
    ...
    fpath_out = "/glade/derecho/scratch/hcluo/q800/"
    begin = int(sys.argv[1])
    years = [begin]

    fout1 = fpath_out+"q800_"+f"{years[0]}.nc"
    fout2 = fpath_out+"dqdy800_"+f"{years[0]}.nc"
    ...
    ds5 = read_data(fnames5,BOUNDARY,temporal_resolution,spatial_resolution).sel(level = 800)
    ...
    q = xr.where(mask==1, q,np.nan).sel(time = TIME_mark)
    dqdy = -wf.ddy(q)
    q_mean = q.mean("time",skipna=True)
    dqdy_mean = dqdy.mean("time",skipna=True)
    del q
    gc.collect()

    ds_out1 = xr.Dataset(data_vars = {"q800":(("latitude","longitude"),q_mean.data)},
                        coords = {"latitude":q_mean.latitude,
                                  "longitude":q_mean.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly climatology mean q at 800 hPa",
                                 "period": str(years[0])+" to "+str(years[-1])})
    ds_out1.to_netcdf(path = fout1) 
    del q_mean,ds_out1

    ds_out2 = xr.Dataset(data_vars = {"dqdy800":(("latitude","longitude"),dqdy_mean.data)},
                        coords = {"latitude":dqdy_mean.latitude,
                                  "longitude":dqdy_mean.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly climatology mean dqdy at 800 hPa",
                                 "period": str(years[0])+" to "+str(years[-1])})
    ds_out2.to_netcdf(path = fout2) 
    del dqdy_mean,ds_out2
