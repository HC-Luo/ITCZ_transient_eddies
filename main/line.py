# line plots flux, precip changes with latitude
import numpy as np 
import xarray as xr
...
def read_data2(fnames, BOUNDARY,temporal_resolution,spatial_resolution):
    ds = xr.open_mfdataset(fnames,concat_dim = 'time', combine='nested',decode_times=False)
    #slicing and regridding
    if (len(ds.latitude) > 1) & (ds.latitude[0] > ds.latitude[1]):
        ds = ds.reindex(latitude=ds.latitude[::-1])
    builtin_spatial_resolution = (ds.latitude[1]-ds.latitude[0]).values
...
if __name__ == "__main__":
    ...
    for i in years:
        ...
        fname2 = "vMSE_col_MMC/vMSE_col_MMC_test_"+str(i)+".nc"
        #fname3 = "vMSE_col_stationary/vMSE_col_stationary_new_"+str(i)+".nc"
        fname3 = "vMSE_col_stationary/vMSE_col_stationary_test_"+str(i)+".nc"
        fname4 = "vMSE_col_transient/vMSE_col_transient_test_"+str(i)+".nc"
    ...
    Data6_mean = Data6_resample.mean(["time","longitude"])
    latitude = Data2_mean.latitude
    data5_lat = precip_centroid(Data5).mean("time").mean("longitude")
    data6_lat = precip_centroid(Data6_resample).mean("time").mean("longitude")
    ...
    dx = ax1.twinx()
    dx.spines['right'].set_position(('axes', 1.2))
    dx.plot(Data6_mean.latitude,Data6_mean,"darkorange",
            label = r"$\overline{[\langle q \rangle]}$", zorder=4)
    dx.plot(data6_lat.values, Data6_mean.sel(latitude = data6_lat,method = 'nearest').values,".",color='darkorange', markersize = 10.0, zorder=5)
    dx.tick_params(axis="both",direction="in")
    dx.set_ylabel(r"$(kg\,m^{-2})$",color = "darkorange",fontsize=13)
    ...
    lines2 = cx.get_lines()
    lines3 = dx.get_lines()
    all_lines = list(lines1) + list(lines2)+list(lines3)
    labels = [line.get_label() for line in all_lines]
    
    # Add shared legend to ax1
    ax1.legend(all_lines, labels, loc='upper left')
    
    plt.savefig("/glade/work/hcluo/v5/line2.pdf",bbox_inches='tight',pad_inches = 0.05)
