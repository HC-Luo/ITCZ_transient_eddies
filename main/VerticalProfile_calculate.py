#calculate vertival profiles of v, MSE, vMSE, v'MSE', on the two sides of EFE
import numpy as np 
import xarray as xr
import myfun as wf
...
def maskout(model,sp):
    if "level" in model.coords:
        ...
        mask = xr.where((level_exp <= sp_exp), Data,np.nan)
...
if __name__ == "__main__":
    ...
    fout1 = fpath_out+"v_"+f"{years[0]}.nc"
    fout2 = fpath_out+"MSE_"+f"{years[0]}.nc"
    fout3 = fpath_out+"vMSE_"+f"{years[0]}.nc"
    fout4 = fpath_out+"vMSE_transient_"+f"{years[0]}.nc"
    fout5 = fpath_out+"v_anno2_"+f"{years[0]}.nc"
    fout6 = fpath_out+"MSE_anno2_"+f"{years[0]}.nc"
    fout7 = fpath_out+"Lvq_anno2_"+f"{years[0]}.nc"
    fout8 = fpath_out+"Lvq_"+f"{years[0]}.nc"
    
    #os.system("rm -r "+fout1)
    #os.system("rm -r "+fout2)
    #os.system("rm -r "+fout3)
    #os.system("rm -r "+fout4)
    #os.system("rm -r "+fout5)
    #os.system("rm -r "+fout6)
...
#
    ...
    mask = maskout(q,sp).sel(time = q.time)#xr.ones_like(vMSE)#
#
    v_adj = xr.where(mask==1, v_adj,np.nan)
    MSE = xr.where(mask==1, MSE,np.nan)
#
    vMSE_full = v_adj*MSE
    vMSE_MMC = (running_mean(v_adj,temporal_resolution).mean("longitude",skipna=True)*running_mean(MSE,temporal_resolution).mean("longitude",skipna=True)).broadcast_like(vMSE_full)
#
    ...
    v_adj_anno2 = v_adj_anno*v_adj_anno
    MSE_anno2 = MSE_anno*MSE_anno
    ...
    q_anno = q-running_mean(q,temporal_resolution)
    q_anno = q_anno-q_anno.mean("longitude",skipna=True)
    q_anno2 = q_anno*q_anno

    v_anno2_mean = xr.where(mask==1, v_adj_anno2,np.nan).sel(time = TIME_mark).mean(["time","latitude","longitude"],skipna=True)
    del v_adj_anno2
    MSE_anno2_mean = xr.where(mask==1, MSE_anno2,np.nan).sel(time = TIME_mark).mean(["time","latitude","longitude"],skipna=True)
    del MSE_anno2
    q_anno2_mean = xr.where(mask==1, q_anno2,np.nan).sel(time = TIME_mark).mean(["time","latitude","longitude"],skipna=True)
    del q_anno2
    q_mean = xr.where(mask==1, q,np.nan).sel(time = TIME_mark).mean(["time","latitude","longitude"],skipna=True)
    del q
    v_mean = xr.where(mask==1, v_adj,np.nan).sel(time = TIME_mark).mean(["time","latitude","longitude"],skipna=True)
    del v_adj
    MSE_mean = xr.where(mask==1, MSE,np.nan).sel(time = TIME_mark).mean(["time","latitude","longitude"],skipna=True)
    del MSE
    vMSE_full_mean = xr.where(mask==1, vMSE_full,np.nan).sel(time = TIME_mark).mean(["time","latitude","longitude"],skipna=True)
    del vMSE_full
    vMSE_transient_mean = xr.where(mask==1, vMSE_transient,np.nan).sel(time = TIME_mark).mean(["time","latitude","longitude"],skipna=True)
    del vMSE_transient
    gc.collect()


    ds_out1 = xr.Dataset(data_vars = {"v_vertical":("level",v_mean.data)},
                        coords = {"level":v_mean.level},
                        attrs = {"description":str(temporal_resolution)+" hourly vertical profile of v",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"m s^-1"})
    ds_out1.to_netcdf(path = fout1) 

    ds_out2 = xr.Dataset(data_vars = {"MSE_vertical":("level",MSE_mean.data)},
                        coords = {"level":MSE_mean.level},
                        attrs = {"description":str(temporal_resolution)+" hourly vertical profile of MSE",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"J kg^-1"})
    ds_out2.to_netcdf(path = fout2) 

    ds_out3 = xr.Dataset(data_vars = {"vMSE_full_vertical":("level",vMSE_full_mean.data)},
                        coords = {"level":vMSE_full_mean.level},
                        attrs = {"description":str(temporal_resolution)+" hourly vertical profile of vMSE",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"W m kg^-1"})
    ds_out3.to_netcdf(path = fout3) 
#
    ds_out4 = xr.Dataset(data_vars = {"vMSE_transient_vertical":("level",vMSE_transient_mean.data)},
                        coords = {"level":vMSE_transient_mean.level},
                        attrs = {"description":str(temporal_resolution)+" hourly vertical profile of vMSE (transient eddy)",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"W m kg^-1"})
    ds_out4.to_netcdf(path = fout4) 
#
    ds_out5 = xr.Dataset(data_vars = {"v_anno2_vertical":("level",v_anno2_mean.data)},
                        coords = {"level":v_anno2_mean.level},
                        attrs = {"description":str(temporal_resolution)+" hourly vertical profile of eddy v variance",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"m^2 s^-2"})
    ds_out5.to_netcdf(path = fout5) 
#
    ds_out6 = xr.Dataset(data_vars = {"MSE_anno2_vertical":("level",MSE_anno2_mean.data)},
                        coords = {"level":MSE_anno2_mean.level},
                        attrs = {"description":str(temporal_resolution)+" hourly vertical profile of eddy MSE variance",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"J^2 kg^-2"})
    ds_out6.to_netcdf(path = fout6) 

    ds_out7 = xr.Dataset(data_vars = {"Lvq_anno2_vertical":("level",Lv*q_anno2_mean.data)},
                        coords = {"level":q_anno2_mean.level},
                        attrs = {"description":str(temporal_resolution)+" hourly vertical profile of eddy Lv*q variance",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"J^2 kg^-2"})
    ds_out7.to_netcdf(path = fout7) 

    ds_out8 = xr.Dataset(data_vars = {"Lvq_vertical":("level",Lv*q_mean.data)},
                        coords = {"level":q_mean.level},
                        attrs = {"description":str(temporal_resolution)+" hourly vertical profile of Lv*q",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"J kg^-1"})
    ds_out8.to_netcdf(path = fout8) 
