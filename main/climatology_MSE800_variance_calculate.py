#Composites of MSE variance at 800
import numpy as np 
import xarray as xr
import myfun as wf
...
if __name__ == "__main__":
    ...
    fpath_out = "/glade/derecho/scratch/hcluo/MSE800_variance/"
    begin = int(sys.argv[1])
    years = [begin]

    fout1 = fpath_out+"MSE800_variance_"+f"{years[0]}.nc"
    ...
    MSE_anno2 = xr.where(mask==1, MSE_anno2,np.nan).sel(time = TIME_mark)
    MSE_anno2_mean = MSE_anno2.mean("time",skipna=True).sel(level = 800)
    del MSE_anno2
    gc.collect()

    ds_out1 = xr.Dataset(data_vars = {"MSE800_variance":(("latitude","longitude"),MSE_anno2_mean.data)},
                        coords = {"latitude":MSE_anno2_mean.latitude,
                                  "longitude":MSE_anno2_mean.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly eddy MSE variance at 800 hPa",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"J^2 kg^-2"})
    ds_out1.to_netcdf(path = fout1) 
    del MSE_anno2_mean,ds_out1
