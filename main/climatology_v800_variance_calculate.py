#Composites of v variance at 800
import numpy as np 
import xarray as xr
import myfun as wf
...
if __name__ == "__main__":
    ...
    fpath_out = "/glade/derecho/scratch/hcluo/v800_variance/"
    begin = int(sys.argv[1])
    years = [begin]

    fout1 = fpath_out+"v800_variance_"+f"{years[0]}.nc"
...
#
    ...
    v_anno2 = xr.where(mask==1, v_adj_anno2,np.nan).sel(time = TIME_mark)
    v_anno2_mean = v_anno2.mean("time",skipna=True).sel(level = 800)
    del v_adj_anno2
    gc.collect()

    ds_out1 = xr.Dataset(data_vars = {"v800_variance":(("latitude","longitude"),v_anno2_mean.data)},
                        coords = {"latitude":v_anno2_mean.latitude,
                                  "longitude":v_anno2_mean.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly eddy v variance at 800 hPa",
                                 "period": str(years[0])+" to "+str(years[-1]),
                                 "unit":"m^2 s^-2"})
    ds_out1.to_netcdf(path = fout1) 
    del v_anno2_mean,ds_out1
