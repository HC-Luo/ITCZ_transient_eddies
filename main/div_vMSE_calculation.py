#calculate divergent component of full MSE flux
import numpy as np 
import xarray as xr
import myfun as wf
...
if __name__ == "__main__":
    ...
    fout1 = fpath_out+"vMSE_col/div_vMSE_col_"+f"{years[0]}.nc"
    ...
    uMSE_full_col = column_integrate(uMSE_full,mask).sel(time = TIME_mark)
    vMSE_full_col = column_integrate(vMSE_full,mask).sel(time = TIME_mark)
    del vMSE_full, uMSE_full,mask
    gc.collect()
    ...
    ds_out1 = xr.Dataset(data_vars = {"div_vMSE_col":(("time","latitude","longitude"),div_vMSE_col.data)},
                        coords = {"time":div_vMSE_col.time,
                                  "latitude":div_vMSE_col.latitude,
                                  "longitude":div_vMSE_col.longitude},
                        attrs = {"description":str(temporal_resolution)+" hourly column-integrated irrotational component of meridional moist static energy flux (full)",
