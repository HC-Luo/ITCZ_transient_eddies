...
def colormap(color,*args, **kwargs):
    ...
    if color == "PurOra":
        ...
        color = ['#ffffff', '#fff7e7', '#ffeed1', '#ffe5be', '#ffdcab', '#ffd29b', '#ffc88c', '#ffbe7f', '#ffb373', '#ffa869', '#ff9d5f', '#ff9157', '#ff854f', '#ff7749', '#ff6942', '#fc5b3c', '#fa4c36', '#f83a2e', '#f42423', '#ef0000']
        cmap  = ListedColormap(color)
        cmap.set_under(color = '#ffffff' )
        cmap.set_over(color = '#f3633e')
    elif color == "WhGreBlu":
...
def sigtest(idx,x,x_regress,adjusted=False,dim = "time"):
    ...
    Cxx = acf(idx,adjusted = adjusted, nlags = nlag)
    if len(m1) == 2:
        Cyy = np.empty((nlag+1,m1[1]))
        #Cyy = np.empty((nlag,m1[1],m1[2]))
        Tau = np.empty(m1[1])
        for i in range(m1[1]):
                Cyy[:,i] = acf(x[:,i],adjusted = adjusted, nlags = nlag)
                Tau[i] = 2*np.sum(Cxx*Cyy[:,i],axis = 0)-1.0
    elif len(m1) == 3:
        Cyy = np.empty((nlag+1,m1[1],m1[2]))
        #Cyy = np.empty((nlag,m1[1],m1[2]))
        Tau = np.empty((m1[1],m1[2]))
        for i in range(m1[1]):
            for j in range(m1[2]):
                Cyy[:,i,j] = acf(x[:,i,j],adjusted = adjusted, nlags = nlag)
                Tau[i,j] = 2*np.sum(Cxx*Cyy[:,i,j],axis = 0)-1.0
    elif len(m1) == 4:
        Cyy = np.empty((nlag+1,m1[1],m1[2],m1[3]))
        #Cyy = np.empty((nlag,m1[1],m1[2]))
        Tau = np.empty((m1[1],m1[2],m1[3]))
        for i in range(m1[1]):
            for j in range(m1[2]):
                for k in range(m1[3]):
                    Cyy[:,i,j,k] = acf(x[:,i,j,k],adjusted = adjusted, nlags = nlag)
                    Tau[i,j,k] = 2*np.sum(Cxx*Cyy[:,i,j,k],axis = 0)-1.0

    #Tau = 2*np.dot(Cxx.T,Cyy)-1.0
...
def regression(idx,x,test,*args, **kwargs):
    idx = (idx-idx.mean(dim = "time"))/idx.std(dim = "time")
...
def spacetime_power(data, segsize, noverlap, spd, dosymmetries=True, rmvLowFrq=True):
    ...
    if dosymmetries:
        ...
        z_final = z_pee.isel(latitude=lat_inds).mean(dim='time').sum(dim='latitude').squeeze()
    return z_final
